...
import { Attendance, AttendanceFormData, AvulsoFormData, ATTENDANCE_STATUS_CONFIG, PendingMakeup } from '../../core/models/attendance.model';
...
export class AttendanceComponent implements OnInit {
  ...
  makeupsPorAluno = computed(() => {
    ...
    if (!patient) return [];
    return this.pendingMakeups().filter(
        ...
        this.patientService.getAttendanceByDate(dateStr).subscribe({
          next: (attendanceOfDay) => {
            ...
            const repostoExtras: PatientAttendance[] = regulares
              .filter(a =>
                a.makeup_origin_id &&
                !idsComAulaHoje.has(String(a.patient_id))
              )
              .map(att => {
      ...
      next: (att) => {
        ...
        this.patientService.getPendingMakeupsList(dateStr).subscribe({
          next: (list) => {
            ...
            const pendentesDoAluno = list.filter(m => String(m.patient_id) === String(patient.id));
  ...
  confirmReposto(): void {
    ...
    this.patientService.resolveReposto({
      makeupId,
    ...
    this.patientService.deleteAttendance(patient.id, prevAttId).subscribe({
      ...
      error: () => {
        ...
        if (wasReposto && !patient.hasClass) {
          ...
          this.patients.update(list => [...list, { ...patient, todayStatus: prevStatus, todayAttendanceId: prevAttId }]);
        }
  ...
  saveAvulso(): void {
    if (this.avulsoForm.invalid) { this.avulsoForm.markAllAsTouched(); return; }
  ...
  deleteAvulso(patient: PatientAttendance): void {
    ...
    this.patientService.deleteAttendance(patient.id, patient.todayAttendanceId).subscribe({
      next: () => {
        ...
        this.applyFilters();
        this.messageService.add({ severity: 'success', summary: 'Removida', detail: `Aula avulsa de ${patient.nome} excluída` });
      },
      error: () => {
        this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Não foi possível excluir a aula avulsa' });
      }
    });
  }
